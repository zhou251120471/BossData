...
根据命令行参数决定评估方式，三种模式互斥，优先级：`use_local > use_llm > 规则匹配`
...
if use_local:
    ...
    if result:
        return result
# 模式3: 规则匹配兜底（仅当LLM评估全部失败时使用）
```

**重要说明**：
- **`--llm` 模式**：仅使用远程大模型评估，不会执行规则匹配逻辑
- **`--local` 模式**：仅使用本地大模型评估，不会执行规则匹配逻辑
- **规则匹配**：仅作为LLM评估失败时的降级方案，不建议主动使用

#### 1.3 规则匹配兜底逻辑

当LLM评估失败或响应解析失败时，系统自动回退到基于关键词匹配的规则评估：

```
1. 获取岗位评估标准中的所有达标项（标准项列表）
2. 在简历全文（full_text + skills + summary）中搜索这些达标项
3. 每个维度得分 = 匹配项数（最大5项）
4. 加权得分 = (维度得分 / 5) × 维度权重
5. 综合匹配度 = 所有维度加权得分之和（最大值100）
6. 根据分数确定优先级：
   - ≥85分 → P0（优先推荐）
   - ≥70分 → P1（推荐）
   - ≥50分 → P2（备选）
...
**评估流程**：
```
1. 构建评估Prompt（包含评估标准 + 候选人信息 + 评估指令）
2. 调用远程LLM API（qwen-max模型，temperature=0.1）
3. 解析LLM返回的JSON格式评估结果
4. 根据评估标准中的优先级等级配置建议动作
5. 返回评估结果
```
...
【岗位评估标准】
一、评估维度及权重：
  D1. 核心技术栈匹配度（权重30%）：精通Vue3+TypeScript...
...
【候选人信息】
姓名、性别、年龄、手机号、应聘岗位、工作经验、学历、核心技能、项目经验摘要...

【评估指令】
请根据评估标准对候选人进行综合评估，输出JSON格式结果...
```

**优点**：
- 深度语义分析，能理解上下文和技术栈的关联性
- 可以识别"Vue2"与"Vue3"的差异，"了解TypeScript"与"精通TypeScript"的区别
- 能评估项目复杂度、工程化能力等抽象维度
- 生成详细的核心优势、主要风险和定级理由

**注意事项**：
- 大模型存在轻微随机性（temperature=0.1），相同候选人多次评估可能有5-10分差异
- 评估结果依赖于简历内容的完整性和准确性
- 需要联网调用API，存在网络延迟
...
当LLM评估失败或响应解析失败时，系统自动回退到基于关键词匹配的规则评估。**此模式仅作为降级方案，不建议主动使用。**

**评估流程**：
```
1. 获取岗位评估标准中的所有达标项（标准项列表）
2. 在简历全文（full_text + skills + summary）中搜索这些达标项的完整字符串
3. 每个维度得分 = 匹配项数（最大5项）
4. 加权得分 = (维度得分 / 5) × 维度权重
5. 综合匹配度 = 所有维度加权得分之和（最大值100）
6. 根据分数确定优先级
...
**缺点**：
- 仅支持完整字符串匹配，无法理解语义关联性
- 无法区分"Vue2"与"Vue3"的差异
- 无法评估项目复杂度、工程化能力等抽象维度
- 评估结果过于简单，缺少详细分析

**评估维度**：
维度来自评估标准文件解析，权重总和为100%：
- 核心技术栈匹配度（权重30%）
- 经验年限与学历（权重20%）
- 项目复杂度与产出（权重25%）
- 工程化与架构能力（权重25%）
